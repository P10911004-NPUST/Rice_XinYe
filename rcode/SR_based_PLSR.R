...
widen_SR <- SR_based %>% 
    pivot_wider(names_from = CHN, values_from = SR_LCC2:SR_LCC5)
...
if(crop_season_year == "2021"){
    joinSR <- filter(joinSR, DATE < "2022-01-01") 
}
...
# ------------------------ PLSR --------------------------------
plsr_mod <- plsr(N ~ ., data = plsr_data,
...
write.csv(PVE, row.names = F,
          file = paste(save_figure_path, "SR_PLSR_PVE.csv", sep = "/"))
...
splitList <- createDataPartition(y = joinSR$row_num,
                                 times = 1, p = .6, list = F)

training <- joinSR[splitList, ] %>% 
    select(-DATE, -DAT, -TREAT, -NO, -row_num)

testing <- joinSR[-splitList, ]%>% 
    select(-DATE, -DAT, -TREAT, -NO, -row_num)

plsr_mod_1 <- plsr(N ~ ., data = training, 
                 ncomp = select_ncomp, scale = T)

val <- predict(plsr_mod_1, newdata = testing[, 1:ncol(testing)-1], ncomp = select_ncomp)
...
ggsave(filename = paste0("SR_based_PLSR_", crop_season_year, ".", plot_device),            
