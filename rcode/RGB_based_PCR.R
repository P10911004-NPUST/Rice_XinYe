...
crop_season_year <- "2021_2022"       # "2021" for single year or "2021_2022"
...
RGB_data <- lcc_data %>% 
    filter(LCC == 6) %>% 
    group_by(DATE, DAT, TREAT, NO, CHN) %>%    # Average the sampling points
    summarise(VALUE = mean2(VALUE)) %>% 
    pivot_wider(names_from = CHN, values_from = VALUE)
...
if(crop_season_year == "2021"){
    joinRGB <- filter(joinRGB, DATE < "2022-01-01") 
}
...
# ------------------------ PCR --------------------------------
pcr_mod <- pcr(N ~ Blue + Green + Red, data = joinRGB,
...
write.csv(PVE, row.names = F,
          file = paste(save_figure_path, "RGB_PCR_PVE.csv", sep = "/"))
...
splitList <- createDataPartition(y = joinRGB$row_num,
                                 times = 1, p = .6, list = F)
training <- joinRGB[splitList, ]
testing <- joinRGB[-splitList, ]

pcr_mod_1 <- pcr(N ~ Blue + Green + Red, data = training, 
                 ncomp = select_ncomp, scale = T)

val <- predict(pcr_mod_1, newdata = testing[, 5:7], ncomp = select_ncomp)
...
#plot_data <- plot_data %>% 
#    filter(X > mean(X) - sd(X)*2 & X < mean(X) + sd(X)*2) %>% 
#    filter(Y > mean(Y) - sd(Y)*2 & Y < mean(Y) + sd(Y)*2)
...
xmin = round(min(val_table$Observed) - abs(min(val_table$Observed) / 100), 2)
xmax = round(max(val_table$Observed) + abs(max(val_table$Observed) / 100), 2)
xinterval = abs((xmax - xmin) / 5)

ymin = round(min(val_table$Predicted) - abs(min(val_table$Predicted) / 100), 2)
ymax = round(max(val_table$Predicted) + abs(max(val_table$Predicted) / 100), 2)
...
ggsave(filename = paste0("RGB_based_PCR_", crop_season_year, ".", plot_device),            
