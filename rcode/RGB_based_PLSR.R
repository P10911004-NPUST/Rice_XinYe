...
# ------------------------ PLSR --------------------------------
plsr_mod <- plsr(N ~ Blue + Green + Red, data = joinRGB,
                 scale = T, validation = 'CV')

summary(plsr_mod)
plot(explvar(plsr_mod))

PVE <- data.frame(PVE = explvar(plsr_mod))

PVE <- PVE %>% 
    mutate(PC = row_number(),
           cumPVE = cumsum(PVE))
write.csv(PVE, row.names = F,
          file = paste(save_figure_path, "RGB_PLSR_PVE.csv", sep = "/"))
...
plsr_mod_1 <- pcr(N ~ Blue + Green + Red, data = training, 
                  ncomp = 1, scale = T)

val <- predict(plsr_mod_1, newdata = testing[, 5:7], ncomp = 1)
...
ggsave(filename = paste0("RGB_based_PLSR_", crop_season_year, ".", plot_device),            
